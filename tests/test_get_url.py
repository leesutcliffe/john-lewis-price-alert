...
def test_it_returns_price_when_url_is_requested():
    ...
    def _get_content():
        with open("config/ercol_page.html", "rb") as f:
            content = f.read()
        return content
    ...
    actual = scraper.get_price(test_url, mocked_request)
...
def test_save_price_pandas_dataframe():
    test_data = {"Date": [datetime.datetime(2022,1,1), datetime.datetime(2022,1,1)], "Price": [450.0, 450.0]}
    test_df = pd.DataFrame(data=test_data)

    actual = scraper.save_current_price(test_df, 450.0)
    assert actual == test_df
